...
export class BasicInformationComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.subscriptions = [
      ...
      this.menuBehaviorSubject.menuBehaviorSubject$.pipe(
        distinctUntilChanged()
      ).subscribe((_) => {
        // 適用が変更されたとき
        this.onInitData();
      }),
      this.menuService.checkedRadio$.pipe(
        distinctUntilChanged()
      ).subscribe((_) => {
        // 仕様が変更されたとき
        this.onInitData();
  ...
  private onInitData() {
    ...
    this.options3 = {
      ...
      locale: 'jp',
      numberCell: { show: true }, // 行番号
      colModel: this.menuService.selectedRoad ? columnHeaderDisableds : columnHeaders,
