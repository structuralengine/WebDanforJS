...
@Component({
  ...
  styleUrls: [
    "./safety-factors-material-strengths.component.scss",
    "../subNavArea.scss",
  ],
})
  ...
  implements OnInit, OnDestroy, AfterViewInit
{
  ...
  public range = {
    0: { text: this.translate.instant("safety-factors-material-strengths.av") },
    1: {
      text: this.translate.instant("safety-factors-material-strengths.rsb_ten"),
    },
    2: {
      text: this.translate.instant("safety-factors-material-strengths.ten_com"),
    },
    3: {
      text: this.translate.instant("safety-factors-material-strengths.all"),
    },
  };
  public rangeArray = [
    {
    ...
    for (let i = 0; i < safety.groupe_list.length; i++) {
      ...
      for (const col of safety.safety_factor[id]) {
        ...
        bar.push({
          ...
          title: col.title,
          M_rc:
            col.M_rc === null || col.M_rc === 0 ? defaultItem.M_rc : col.M_rc,
          M_rs:
            col.M_rs === null || col.M_rs === 0 ? defaultItem.M_rs : col.M_rs,
          M_rbs:
            col.M_rbs === null || col.M_rbs === 0
              ? defaultItem.M_rbs
              : col.M_rbs,
          V_rc:
            col.V_rc === null || col.V_rc === 0 ? defaultItem.V_rc : col.V_rc,
          V_rs:
            col.V_rs === null || col.V_rs === 0 ? defaultItem.V_rs : col.V_rs,
          V_rbc:
            col.V_rbc === null || col.V_rbc === 0
              ? defaultItem.V_rbc
              : col.V_rbc,
          V_rbs:
            col.V_rbs === null || col.V_rbs === 0
              ? defaultItem.V_rbs
              : col.V_rbs,
          V_rbv:
            col.V_rbv === null || col.V_rbv === 0
              ? defaultItem.V_rbv
              : col.V_rbv,
          T_rbt:
            col.T_rbt === null || col.T_rbt === 0
              ? defaultItem.T_rbt
              : col.T_rbt,
          ri: col.ri === null || col.ri === 0 ? defaultItem.ri : col.ri,
          range: parseInt(
            col.range === null || col.range === 0
              ? defaultItem.range
              : col.range
          ),
          pq_cellstyle: col.pq_cellstyle,
          NoCalc:
            col.NoCalc === null || col.NoCalc === 0
              ? defaultItem.NoCalc
              : col.NoCalc,
        });
        steel.push({
      ...
      this.table5_datas.push([
        {
          title: this.translate.instant(
            "safety-factors-material-strengths.tys"
          ),
          SRCfsyk1: s1.fsyk,
          SRCfsyk2: s2.fsyk,
          SRCfsyk3: s3.fsyk,
        ...
        {
          title: this.translate.instant(
            "safety-factors-material-strengths.sys"
          ),
          SRCfsyk1: s1.fsvyk,
          SRCfsyk2: s2.fsvyk,
          SRCfsyk3: s3.fsvyk,
      ...
      this.table3_datas.push([
        {
          title: this.translate.instant(
            "safety-factors-material-strengths.fck"
          ),
          value: concrete.fck,
        },
        {
      ...
      this.option1_list.push({
        ...
        change(evt, ui) {
          for (const updateItem of ui.updateList) {
            let defaultItem = self.default_factor.filter(
              (x) => x.id === updateItem.rowData.id
            )[0];
            for (let key in updateItem.newRow) {
              ...
              if (
                updateItem.newRow[key] === null ||
                updateItem.newRow[key] === undefined
              ) {
        ...
        change: (evt, ui) => {
          ...
          for (const updateItem of ui.updateList) {
            for (let key in updateItem.newRow) {
              ...
              if (
                updateItem.newRow[key] === 0 &&
                ((!this.allowedValues1.includes(updateItem.oldRow[key]) &&
                  updateItem.rowIndx === 0) ||
                  (!this.allowedValues2.includes(updateItem.oldRow[key]) &&
                    updateItem.rowIndx !== 0))
              ) {
                updateItem.newRow[key] = parseFloat(updateItem.oldRow[key]);
                updateItem.rowData[key] = parseFloat(updateItem.oldRow[key]);
  ...
  private setTitle(): void {
    this.columnHeaders1 = [
      ...
      {
        title: this.translate.instant(
          "safety-factors-material-strengths.execute"
        ),
        align: "center",
        dataType: "bool",
        dataIndx: "NoCalc",
      ...
      {
        title: this.translate.instant(
          "safety-factors-material-strengths.b_safe"
        ),
        align: "center",
        colModel: [
          {
            title: this.translate.instant(
              "safety-factors-material-strengths.material_coefficient"
            ),
            align: "center",
            colModel: [
              {
      ...
      {
        title: this.translate.instant(
          "safety-factors-material-strengths.s_safe"
        ),
        align: "center",
        colModel: [
          {
            title: this.translate.instant(
              "safety-factors-material-strengths.t_safe1"
            ),
            align: "center",
            colModel: [
              {
          ...
          {
            title: this.translate.instant(
              "safety-factors-material-strengths.t_safe2"
            ),
            align: "center",
            colModel: [
              {
      ...
      {
        title: this.translate.instant(
          "safety-factors-material-strengths.rsb_arr"
        ),
    ...
    this.columnHeaders3 = [
      ...
      // { title: '', dataType: 'float', dataIndx: 'value', sortable: false, width: 140, nodrag: true, },
      {
        ...
        editor: {
          type: "select",
          options: (data) =>
            data.rowIndx === 0 ? this.optionsConcrete1 : this.optionsConcrete2,
          labelIndx: "text",
          valueIndx: "id",
        },
        render: (ui) => {
          ...
          if (this.allowedValues2.includes(value) && ui.rowIndx !== 0) {
            ...
            return this.translate.instant(
              "safety-factors-material-strengths.av"
            );
          }
        },
      },
  ...
  public saveData(): void {
    ...
    for (let i = 0; i < this.groupe_list.length; i++) {
      ...
      for (let j = 0; j < safety_bar.length; j++) {
        const bar = safety_bar[j],
          steel = safety_steel[j];
        factor.push({
          id: bar.id,
          title: bar.title,
          M_rc: bar.M_rc,
  ...
  addStyleIfValueAllowed(data: any, id: any) {
    ...
    data.forEach((data: any) => {
      ...
      const isValueAllowed =
        this.allowedValues1.includes(value) ||
        this.allowedValues2.includes(value);
  ...
  handleSetSelect(dataTable: any, id: any) {
    ...
    dataTable.forEach((data: any) => {
      ...
      for (let i = 0; i < fx.length; i++) {
        ...
        if (
          (data[k1] === 295 && data[k2] === 440) ||
          (data[k1] === 295 && data[k2] === null) ||
          (data[k1] === null && data[k2] === 440)
        ) {
          data[`options${i + 1}`] = "1";
        ...
        if (
          (data[k1] === 345 && data[k2] === 490) ||
          (data[k1] === 345 && data[k2] === null) ||
          (data[k1] === null && data[k2] === 490)
        ) {
          data[`options${i + 1}`] = "2";
          this.setEdit(data, false, k1, k2);
          data[k1] = 345;
        ...
        if (
          (data[k1] === 390 && data[k2] === 560) ||
          (data[k1] === 390 && data[k2] === null) ||
          (data[k1] === null && data[k2] === 560)
        ) {
          data[`options${i + 1}`] = "3";
          this.setEdit(data, false, k1, k2);
          data[k1] = 390;
        ...
        if (
          (data[k1] === 490 && data[k2] === 620) ||
          (data[k1] === 490 && data[k2] === null) ||
          (data[k1] === null && data[k2] === 620)
        ) {
          data[`options${i + 1}`] = "4";
          this.setEdit(data, false, k1, k2);
          data[k1] = 490;
